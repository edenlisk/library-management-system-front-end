        ...
        const LessBooks = ({BKname, total, remaining, id, category}) => {
            ...
            return (
                <Box padding={4.5} display="flex" gap={0.5} alignItems="center">
                    ...
                    <Typography variant="h5">Book:<span style={{fontWeight: "bolder"}}> {BKname}</span> of category:<span
                        style={{fontWeight: "bold"}}> {category}</span> is at it's limit <span
                        style={{color: "red"}}>{`(${remaining} of ${total})`}</span> is remaining.</Typography>
                </Box>
            )
        }
        return (
            ...
            <>
                ...
                <Box padding={3.5}>
                    {isLoading ? <CircularProgress size={25}/> : <Accordion sx={{
                        backgroundColor: theme.palette.background.default,
                        '& .MuiAccordionDetails-root': {padding: 0},
                        '& .MuiBox-root': {padding: 1}
                    }}>
                        <AccordionSummary
                        ...
                        <AccordionDetails>
                            {bookes.map(({
                                             bookName, numberOfBooks, availableCopy, _id, categoryName
                                         }) => (<LessBooks key={_id}
                                                           BKname={bookName} total={numberOfBooks} remaining={availableCopy}
